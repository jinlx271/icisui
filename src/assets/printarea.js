export default class {
    ...
    init() {
        ...
        // if (this.settings.endCallback) { // 解决不传此参数报错的问题
    ...
    getHead() {
        ...
        [].forEach.call(document.querySelectorAll('link'), function (item, i) {
            if (item.href.indexOf('.css') >= 0 && item.getAttribute('rel') !== 'prefetch') {
        ...
        return `<head><title>${this.settings.popTitle}</title>${links}<style type="text/css">${style}</style>${extraHead}</head>`;
    }
    getBody() {
