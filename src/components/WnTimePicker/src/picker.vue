  ...
  methods: {
    ...
    handleFocus() {
      ...
      if (this.$refs.reference.$el) {
        setTimeout(() => {
          this.$refs.reference.$el.children[0].selectionStart = 0
          this.$refs.reference.$el.children[0].selectionEnd = 2
        }, 0)
      }
      this.clickNubmer = 0
    },
    ...
    mountPicker() {
      ...
      const updateOptions = () => {
        ...
        for (const option in options) {
          if (options.hasOwnProperty.call(option) && option !== 'selectableRange') {
    ...
    handleClick(e) {
      ...
      } else if (e.target.selectionStart > 3 && e.target.selectionStart < 5) {
        e.target.selectionStart = 3
        e.target.selectionEnd = 5
      } else if (e.target.selectionStart > 4) {
    ...
    handleInput(value) {
      ...
      if (this.clickNubmer > 1) {
        if (value.indexOf(':') !== -1 && value.split(':').length > 0) {
          if (this.$refs.reference?.$el?.children[0].selectionStart == 2) {
            this.$refs.reference.$el.children[0].selectionStart = 3
            this.$refs.reference.$el.children[0].selectionEnd = 5
          } else if (
            this.$refs.reference?.$el?.children[0].selectionStart == 5
          ) {
            this.$refs.reference.$el.children[0].selectionStart = 6
            this.$refs.reference.$el.children[0].selectionEnd = 8
          }
