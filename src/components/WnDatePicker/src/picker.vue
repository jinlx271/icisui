<template>
  <el-input class="el-date-editor" :class="'el-date-editor--' + type"
    :readonly="!editable || readonly || type === 'dates' || type === 'week'" :disabled="pickerDisabled" :size="pickerSize"
    :name="name" v-bind="firstInputId" v-if="!ranged" v-clickoutside="handleClose" :placeholder="placeholder"
    @focus="handleFocus" @keydown.native="handleKeydown" :value="displayValue" @input="handleInput" @change="handleChange"
    @click.native.prevent="handleClick" @mouseenter.native="handleMouseEnter" @mouseleave.native="showClose = false"
  ...
  <div class="el-date-editor el-range-editor el-input__inner" :class="[
    ...
    <input autocomplete="off" :placeholder="startPlaceholder" :value="displayValue && displayValue[0]"
      :disabled="pickerDisabled" v-bind="firstInputId" :readonly="!editable || readonly" :name="name && name[0]" @keydown="handleKeydown"
    ...
    <input autocomplete="off" :placeholder="endPlaceholder" :value="displayValue && displayValue[1]"
      :disabled="pickerDisabled" v-bind="secondInputId" :readonly="!editable || readonly" :name="name && name[1]" @keydown="handleKeydown"
    ...
    <i @click="handleClickIcon" v-if="haveTrigger" :class="[showClose ? '' + clearIcon : '']"
      class="el-input__icon el-range__close-icon"></i>
  </div>
</template>
...
// import Popper from 'element-ui/src/utils/vue-popper'//改PopupManager.zIndex问题
...
export default {
  ...
  methods: {
    ...
    handleClick(e) {
      ...
      } else if (e.target.selectionStart > 10 && e.target.selectionStart < 13) {
        e.target.selectionStart = 11
        e.target.selectionEnd = 13
      } else if (e.target.selectionStart > 13 && e.target.selectionStart < 16) {
        e.target.selectionStart = 14
        e.target.selectionEnd = 16
      } else if (e.target.selectionStart > 16) {
        e.target.selectionStart = 17
        e.target.selectionEnd = 19
      }
      this.clickNubmer = 0
    },
    ...
    handleInput(value) {
      ...
      if (this.clickNubmer > 1) {
        if (value.indexOf(':') !== -1 && value.split(':').length > 0) {
          if (this.$refs.reference?.$el?.children[0].selectionStart == 4) {
            this.$refs.reference.$el.children[0].selectionStart = 5
            this.$refs.reference.$el.children[0].selectionEnd = 7
          } else if (this.$refs.reference?.$el?.children[0].selectionStart == 7) {
            this.$refs.reference.$el.children[0].selectionStart = 8
            this.$refs.reference.$el.children[0].selectionEnd = 10
          } else if (this.$refs.reference?.$el?.children[0].selectionStart == 10) {
            this.$refs.reference.$el.children[0].selectionStart = 11
            this.$refs.reference.$el.children[0].selectionEnd = 13
          } else if (this.$refs.reference?.$el?.children[0].selectionStart == 13) {
            this.$refs.reference.$el.children[0].selectionStart = 14
            this.$refs.reference.$el.children[0].selectionEnd = 16
          } else if (this.$refs.reference?.$el?.children[0].selectionStart == 16) {
            this.$refs.reference.$el.children[0].selectionStart = 17
            this.$refs.reference.$el.children[0].selectionEnd = 19
          }
          const el = this.$refs.reference.$el.children[0]
          const startPos = this.$refs.reference.$el.children[0].selectionStart
          const endPos = this.$refs.reference.$el.children[0].selectionEnd
          ...
          } else if (el.value.substring(startPos, endPos) == ' :' || el.value.substring(startPos, endPos) == '- ' || el.value.substring(startPos, endPos) == '--') {
            el.selectionStart = endPos - 2
            el.selectionEnd = endPos - 2
            el.setSelectionRange(endPos - 2, endPos - 2)
          } else if (el.value.substring(startPos, endPos).indexOf(':') > -1 || el.value.substring(startPos, endPos).indexOf('-') > -1 || el.value.substring(startPos, endPos).indexOf(' ') > -1) {
            el.selectionStart = endPos - 2
            el.selectionEnd = endPos - 2
            el.setSelectionRange(endPos - 2, endPos - 2)
          }
    ...
    handleKeydown(event) {
      ...
      if (keyCode == 8) {
        ...
        if (range.isCollapsed && (event.target.value.substring(endPos - 1, endPos) == ':' || event.target.value.substring(endPos - 1, endPos) == '-' || event.target.value.substring(endPos - 1, endPos) == ' ')) { // 未选中内容 单纯的删除
          ...
          if (endPos - startPos == event.target.value.length) {
            return
          } else {
            event.stopPropagation()
            event.preventDefault()
    ...
    mountPicker() {
      ...
      if (!this.isShowPickerPanel) {
        this.picker.popperClass += ' hidden-panel'
      }
