...
export default {
  ...
  methods: {
    ...
    pathFc() {
      ...
      // })
      this.routes = JSON.parse(JSON.stringify(this.mainMenu))
    },
    ...
    // 过滤父层元素
    filterRootRouter(menuPathList) {
      if (menuPathList == null || menuPathList.length == 0) {
        return []
      }
      const rootRoutes = []
      const settingRoutes = this.$router.options.settingRoutes
      settingRoutes.forEach(element => {
        ...
        if (menuPathList.indexOf(key) > -1) {
          ...
          if (children && children.length > 0) {
            this.filterRouter(menuPathList, element)
          }
        }
      })
      return rootRoutes
    ...
    filterRouter(menuPathList, element) {
      ...
      element.children = children.filter(subelement => {
        ...
        this.filterRouter(menuPathList, subelement)
        return true
      })
    }
  }
