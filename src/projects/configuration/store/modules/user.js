...
import { shiftListWrap } from '@/utils/utils'
import {
  findShiftWithPage
...
console.log(' user.js  service ', service.defaults.baseURL)
// if (WINMYPlat) {
...
const user = {
  ...
  actions: {
     ...
     * code winMY登录
     */
    WinMYLogin({ commit, dispatch }, loginInfo) {
      // 779501 实现标准网页集成登入接口
      // console.log('12222222222222 WINMYPlat', WINMYPlat)
      groupOrgId = accountInfo.employee.groupOrgId
      orgId = accountInfo.employee.regionOrgId
      return new Promise((resolve, reject) => {
        service({
          url: appKey + '/' + groupOrgId + '/' + orgId + '/sys/h5Login',
          method: 'post',
          data: loginInfo
        }).then((res) => {
            if (res.code == 0) {
