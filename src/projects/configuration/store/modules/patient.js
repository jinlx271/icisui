...
const patient = {
  ...
  mutations: {
    set_timestamp: function (state, patientInfo) {
    ...
    set_patientList: (state, list) => {
      ...
      if (state.patientInfo) {
        state.patientInfo = Object.assign(state.patientInfo, list.filter(v => v.icuRecordId == state.patientInfo.icuRecordId)[0]?.patientInfo)
