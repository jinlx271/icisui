  ...
  <div class="syncPassModifyConfig">
    ...
    <div class="syncPassModifyConfigContent">
      <el-form ref="form" :model="encryptionInfo" label-width="10em" :rules="rules" align="left">
        <el-form-item label="是否启用WIN-GPT" prop="enable" class="line30" >
          <el-switch v-model="encryptionInfo.enable" active-value="1" inactive-value="0" @change="changeValidate"></el-switch>
        </el-form-item>
          <el-form-item label="URL" prop="url">
            <el-input v-model="encryptionInfo.url" placeholder="请输入" :maxlength="200" ></el-input>
          </el-form-item>
          <el-form-item label="API-KEY" prop="key">
            <el-input v-model="encryptionInfo.key" placeholder="请输入" :maxlength="200" ></el-input>
          </el-form-item>
          <el-form-item label="启用的业务场景" prop="scene">
            <el-select v-model="scene" clearable filterable multiple placeholder="请选择" @change="handleChangeScene($event)">
              <el-option v-for="item in ICIS_DICT_GPT_SCENE" :key="item.dictCode" :label="item.dictName" :value="item.dictCode" :disabled="item.statusDict == '0'"></el-option>
            </el-select>
          </el-form-item>
      </el-form>
    </div>
...
export default {
    ...
    string: {
      handler: function (n) {
        try {
          ...
          this.scene = this.encryptionInfo.scene ? this.encryptionInfo.scene.split(',') : []
          this.changeValidate(this.encryptionInfo.enable)
        } catch (error) {
          //
        }
      },
  ...
  methods: {
    ...
    changeValidate(e) {
      if (e == '0') {
        ...
        this.rules = {
          ...
          scene: [
            {
              required: true, message: '请选择启用的业务场景', trigger: 'blur'
            }
          ]
        }
      }
    ...
    handleChangeScene(v) {
      this.encryptionInfo.scene = v.join(',')
    }
  }
}
</script>
