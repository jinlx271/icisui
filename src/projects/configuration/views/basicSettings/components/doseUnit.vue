    ...
    <el-container class="dose-config-right">
      ...
      <el-main class="dose-config-right-body">
        <el-table
          ...
          <v-table-column label="剂量单位" align="center">
            <template slot-scope="scope">
              <el-select v-if="scope.row.isEdit" v-model="scope.row.doseUnit" default-first-option filterable
                placeholder="请选择" style="width:100%">
                <el-option v-for="item in ICIS_DICT_UNIT" :key="item.dictCode" :label="item.dictName"
                  :value="item.dictName">
                </el-option>
              </el-select>
              <span v-else>{{ scope.row.doseUnit }}</span>
            </template>
          </v-table-column>
          <v-table-column label="单位系数" align="center">
            <template slot-scope="scope">
              <el-input v-only-num :min="0" :max="999999999" v-if="scope.row.isEdit" v-model="scope.row.doseUnitCoefficient"
                placeholder="请输入" clearable :disabled="!scope.row.isEdit"> </el-input>
              <span v-else>{{ scope.row.doseUnitCoefficient }}</span>
            </template>
          </v-table-column>
...
<script>
import {
  doseUnitConverter_findWithPage,
  doseUnitConverter_insert,
  doseUnitConverter_edit
...
export default {
  ...
  methods: {
    ...
    async init() {
      ...
      this.ICIS_DICT_UNIT = ICIS_DICT_UNIT
      // 左边字典查询
      this.initData(1)
    },
    // 获取病种目
    initData(page) {
      ...
      this.loading = true
      doseUnitConverter_findWithPage(this.qeuryTableData).then(req => {
        this.loading = false
    ...
    saveItemRow(index, row, callback) {
      if (!row.doseUnit) {
        ...
        return
      } else if (!row.doseUnitCoefficient) {
      ...
      callback && callback()
      if (row.isCreate) {
        doseUnitConverter_insert(row).then(res => {
          row.isEdit = false
          delete row.isCreate
          this.$message({
            type: 'success',
        ...
        doseUnitConverter_edit(row).then(res => {
          row.isEdit = false
          this.$message({
            type: 'success',
            message: '修改成功'
