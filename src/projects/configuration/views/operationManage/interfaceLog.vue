<template>
  <div class="interfaceLog" id="interfaceLog" ref="interfaceLog" element-loading-text="加载中" v-loading="loading">
    <el-container style="height:100%">
      <!-- 头部 -->
      <el-header class="interfaceLogHeader">
        <div class="flex">
          <el-input class="inputclass" v-model="queryData.searchKey" size="small" placeholder="请输入关键字" clearable
            suffix-icon="el-icon-search"></el-input>
          <el-input class="inputclass" v-model="queryData.visitNumber" size="small" placeholder="请输入就诊号" clearable
            suffix-icon="el-icon-search"></el-input>
          <el-input class="inputclass" v-model="queryData.tranCode" size="small" placeholder="请输入服务码" clearable
            suffix-icon="el-icon-search"></el-input>
          <el-select class="inputclass" filterable clearable v-model="queryData.logStatus" placeholder="请选择">
      ...
      <el-main style="height:calc(100% - 112px)">
        <el-table
          ...
          <v-table-column prop="visitNumber" label="就诊号" align="center" width="100">
            <template slot-scope="scope">
              <span>{{ scope.row.visitNumber }}</span>
            </template>
          </v-table-column>
          <v-table-column prop="patientId" label="患者ID" align="center" width="100">
            <template slot-scope="scope">
              <span>{{ scope.row.patientId }}</span>
            </template>
          </v-table-column>
          <v-table-column prop="requestBody" label="请求内容" align="left" min-width="200">
            <template slot-scope="scope">
              <span class="patientNameEllipsis pointer" style="--patient-name-width: 100%;--patient-name-columns: 4;"
                title="点击查看详情" @click="handleShowDetail('请求内容', scope.row.requestBody)">{{
                  scope.row.requestBody }}</span>
            </template>
          </v-table-column>
          <v-table-column prop="responseBody" label="返回内容" align="left" min-width="200">
            <template slot-scope="scope">
              <span class="patientNameEllipsis pointer" style="--patient-name-width: 100%;--patient-name-columns: 4;"
                title="点击查看详情" @click="handleShowDetail('返回内容', scope.row.responseBody)">{{ scope.row.responseBody
                }}</span>
            </template>
          </v-table-column>
          <v-table-column prop="tranCode" label="服务码" align="center" width="100">
            <template slot-scope="scope">
              <span>{{ scope.row.tranCode }}</span>
            </template>
          </v-table-column>
          <v-table-column prop="tranName" label="服务名称" align="center" width="100">
            <template slot-scope="scope">
              <span>{{ scope.row.tranName }}</span>
            </template>
          </v-table-column>
          <v-table-column prop="exceptionContent" label="异常内容" align="center">
            <template slot-scope="scope">
              <span class="patientNameEllipsis pointer" style="--patient-name-width: 100%;--patient-name-columns: 4;"
          ...
          <v-table-column prop="logStatus" label="异常状态" align="center" width="100">
            <template slot-scope="scope">
              <span>{{ scope.row.logStatus == 0 ? '正常' : '错误' }}</span>
            </template>
          </v-table-column>
          <v-table-column prop="createTime" label="创建时间" align="center" width="150">
            <template slot-scope="scope">
...
import { outerLog_findPage } from '@/api/configuration'
...
export default {
  ...
  methods: {
    ...
    queryTableFc(pageNum) {
      ...
      outerLog_findPage(this.queryData).then(res => {
