...
;(function () {
  ...
  var internalSet = function (dom, property, value) {
    ...
    if (isSupported$1(dom)) {
      ...
      dom.setAttribute('data-mce-style', property + ':' + value)
    }
  }
  ...
  var type$1 = Adt.generate([{ none: [] }, { multiple: ['elements'] }, { single: ['element'] }])
  ...
  var cellOperations = function (mutate, doc, formatsToClone) {
    ...
    var newCell = function (prev) {
      ...
      const dataColType = prev.element.dom.getAttribute('data-col-type')
      if (dataColType) {
        //data-col-type 需要复制
        td.dom.setAttribute('data-col-type', dataColType)
      }
  ...
  var adjustWidth = function (table, delta, index, resizing, tableSize, type) {
    ...
    if (type == 'winning' && isLastColumn) {
      if (isLastColumn) {
        deltas = deltas.map((item, colNum) => {
          if (colNum == index) {
            item = step
          } else item = 0
          return item
    ...
    var newWidths = map(deltas, function (dx, i) {
      tableWidth += dx + widths[i]
      return dx + widths[i]
    })
    table.dom.style.width = tableWidth + 'px' // 调整宽度的同时 修改表格的宽度
  ...
  var adt$1 = Adt.generate([{ invalid: ['raw'] }, { pixels: ['value'] }, { percent: ['value'] }])
  ...
  var create$1 = function (wire, resizing, lazySizing) {
    ...
    manager.events.adjustWidth.bind(function (event) {
      ...
      console.log('这里是 || 调整宽度')
      adjustWidth(table, delta, event.column, resizing, tableSize, 'winning')
  ...
  var fireTableSelectionClear = function (editor) {
    editor.fire('TableSelectionClear') // 清除表选择时触发TableModified
  }
  var fireTableModified = function (editor, table, data) {
    editor.fire('TableModified', __assign(__assign({}, data), { table: table })) //当对表格进行样式或结构更改时触发ObjectResized
  }
  var styleModified = {
    structure: false,
    style: true
  ...
  var getTableSizingMode = function (editor) {
    //该table_sizing_mode选项强制执行用于新表和修改表的表大小调整方法（包括对表进行调整大小操作）。此选项仅影响表格和单元格的宽度，不适用于表格和单元格的高度。
    return editor.getParam('table_sizing_mode', 'auto')
  }
  var getCriticlalRecordTypeDict = function (editor) {
    //获取表格组件类型
    return editor.getParam('criticlalRecordTypeDict')
  }
  ...
  var hasTableResizeBars = function (editor) {
    // 此选项可以禁用通过拖动两列或两行之间的边框来调整表格列和行大小的功能。
  ...
  var hasAdvancedCellTab = function (editor) {
    // 此选项可以禁用表格单元格对话框中的高级选项卡。高级选项卡允许用户输入style,border color和background color值。
    return editor.getParam('table_cell_advtab', true, 'boolean')
  }
  var hasAdvancedRowTab = function (editor) {
    // 此选项可以禁用表格行对话框中的高级选项卡。高级选项卡允许用户输入style,border color和background color值。
    return editor.getParam('table_row_advtab', true, 'boolean')
  }
  var hasAdvancedTableTab = function (editor) {
    // 此选项可以禁用表格对话框中的高级选项卡。高级选项卡允许用户输入style,border color和background color值。
    return editor.getParam('table_advtab', true, 'boolean')
  }
  // 此选项允许您在插入或编辑表格时禁用用户可用的某些选项。设置为false以下字段时不会出现：单元格间距、单元格填充、边框和标题
  ...
  var shouldStyleWithCss = function (editor) {
    // 此选项使您可以强制表格属性对话框使用 HTML5/CSS3 标准来设置单元格间距和单元格填充。默认情况下，这些作为属性添加到表元素中。通过将此设置为 true，将单元格间距作为border-spacing样式应用于表格元素，并将单元格填充作为样式应用于所有td元素padding。
    return editor.getParam('table_style_by_css', false, 'boolean')
  }
  var getCellClassList = function (editor) {
    // 此选项使您能够指定要在表格单元格选项对话框中显示的类列表。如果您希望用户将预定义的类分配给表格单元格
    return editor.getParam('table_cell_class_list', [], 'array')
  }
  var getRowClassList = function (editor) {
    // 此选项使您能够指定要在表格行选项对话框中显示的类列表。如果您希望用户将预定义的类分配给表行
    return editor.getParam('table_row_class_list', [], 'array')
  }
  var getTableClassList = function (editor) {
    return editor.getParam('table_class_list', [], 'array') //此选项使您能够指定要在表选项对话框中显示的类列表。如果您希望用户将预定义的类分配给表元素，这很有用。
  }
  ...
  // 此选项将元素添加colgroup到col新表以指定列宽。现有表和使用setContent或粘贴添加的表不受影响
  ...
  var getTableHeaderType = function (editor) {
    ...
    var value = editor.getParam('table_header_type', defaultValue, 'string') // 当表格行设置为标题行时，该table_header_type选项会影响表格的结构
  ...
  var getColumnResizingBehaviour = function (editor) {
    ...
    var givenMode = editor.getParam('table_column_resizing', defaultColumnResizingBehaviour, 'string') //该table_column_resizing选项设置当用户调整、插入或删除表列的大小时是否调整表或其他列的大小
  ...
  var getCloneElements = function (editor) {
    var cloneElements = editor.getParam('table_clone_elements') //此选项使您能够指定在向表中插入行/列时应将哪些元素克隆为空子元素。默认情况下，它会将这些 ' strong em b i span font h1 h2 h3 h4 h5 h6 p div' 克隆到新单元格中。
  ...
  var getResizeHandler = function (editor) {
    ...
    var afterCornerResize = function (table, origin, width, newHeight) {
      var isRightEdgeResize = endsWith(origin, 'e')

      var resizeTableHeight = function (table, newHeight) {
        //原有的表格属性设置只是单纯的调整了表格的style 未对表格的行高进行同步 导致在单独调整 行高是 表格又缩小或者放大
        const warehouse = Warehouse.fromTable(table)
        const rowList = warehouse.all.map((row) => row.element.dom)
        let deltaSum = 0
        const delta = rowList.map((row) => {
          const trH = Number(row.style.height.split('px')[0])
          deltaSum += parseInt(((newHeight - oriTableH) * trH) / newHeight, 10)
          return parseInt(((newHeight - oriTableH) * trH) / newHeight, 10)
        })
        rowList.forEach((row, rIndex) => {
          const trH = parseInt(row.style.height.split('px')[0], 10)

          adjustHeight(table,  delta[rIndex], rIndex, height)
        })
        table.dom.style.height = startH + deltaSum + 'px'
      }
      if (startRawW === '') {
      ...
      if (startH !== newHeight && newHeight !== '') {
        resizeTableHeight(table, newHeight)
      }
    ...
    editor.on('ObjectResizeStart', function (e) {
      //当使用调整大小条在表格、行、列或单元格上启动调整大小操作时触发
      ...
      if (isTable(targetElm)) {
        ...
        startW = e.width
        startH = e.height //新高度
        oriTableH = Number(targetElm.style.height.split('px')[0])
        startRawW = isResponsiveForced(editor) ? '' : getRawWidth$1(editor, targetElm).getOr('')
      }
    })
    editor.on('ObjectResized', function (e) {
      //当使用调整大小条对表格、行、列或单元格完成调整大小操作时触发
      var targetElm = e.target
      if (isTable(targetElm)) {
        var table = SugarElement.fromDom(targetElm)
        var origin_1 = e.origin
        if (startsWith(origin_1, 'corner-')) {
          afterCornerResize(table, origin_1, e.width, e.height)
        }
        removeDataStyle(table)
    ...
    return {
      ...
      lazyWire: lazyWire,
      destroy: destroy,
      lazyResizingBehaviour: lazyResizingBehaviour // 返回调整表格大小的方法
    }
  }
  ...
  var run = function (operation, extract, adjustment, postAction, genWrappers) {
    return function (wire, table, target, generators, sizing, resizeBehaviour) {
      ...
      return output.bind(function (out) {
        ...
        /*** 按照实际宽高 对table进行重新设置 start  */
        var colList = out.grid.find((item) => item.section == 'colgroup').cells
        var rowList = out.grid.filter((item) => item.section == 'tbody')
        var tableWidth = 0
        var tableHeight = 0
        Array.from(colList).forEach((cell) => {
          tableWidth += Number(cell.element.dom.style.width.split('px')[0])
        })
        rowList.forEach((row) => {
          row.cells.forEach((td) => {
            td.element.dom.style.height = row.element.dom.style.height
          })
          tableHeight += Number(row.element.dom.style.height.split('px')[0])
        })
        table.dom.style.width = tableWidth + 'px'
        table.dom.style.height = tableHeight + 'px'
        /*** 按照实际宽高 对table进行重新设置 end  */
        return Optional.some({
          cursor: out.cursor,
    ...
    if (newSectionName !== oldSectionName) {
      ...
      if (!sectionElm_1) {
        ...
        if (newSectionName === 'thead') {
          last(children$2(SugarElement.fromDom(tableElm), 'caption,colgroup')).fold(
            function () {
  ...
  var getCellsFromSelection = function (start, selections, isRoot) {
    return getSelectionStartCell(start, isRoot)
      .map(function (_cell) {
  ...
  var TableActions = function (editor, lazyWire, selections) {
    ...
    var execute = function (operation, guard, mutate, lazyWire, effect) {
      return function (table, target) {
        ...
        return guard(table)
          ? operation(wire, table, target, generators, sizing, resizeBehaviour).bind(function (result) {
  ...
  var insert$1 = function (editor, columns, rows, colHeaders, rowHeaders) {
    ...
    return descendant$1(getBody$1(editor), 'table[data-mce-id="__mce"]')
      .map(function (table) {
        ...
        //兼容老数据 每个单元格为不换行的数据 每个cell 默认的为不换行数据
        each(cells(table), function (cell) {
          var modifier = DomModifier.ifTruthy(editor, cell.dom)
          modifier.setStyle('white-space', 'nowrap')
        })
  ...
  var getApi = function (editor, clipboard, resizeHandler, selectionTargets) {
    return {
      ...
      setClipboardRows: setClipboardElements(clipboard.setRows), // 设置剪贴板行
      getClipboardRows: getClipboardElements(clipboard.getRows), // 获取剪贴板行
      setClipboardCols: setClipboardElements(clipboard.setColumns), // 设置要用于mceTablePasteColBefore或mceTablePasteColAfter用于将列粘贴到表中的数据
      getClipboardCols: getClipboardElements(clipboard.getColumns), // mceTableCutCol返回使用或剪切或复制的任何列的数据
  ...
  //jlx 获取表格的原有colgroup数据
  var getOriColGroup = function (house, minColRange, maxColRange) {
    if (Warehouse.hasColumns(house)) {
      var colsToCopy = filter(Warehouse.justColumns(house), function (col) {
        return col.column >= minColRange && col.column < maxColRange
      })
      var copiedCols = map(colsToCopy, function (c) {
        var clonedCol = c.element
        //constrainSpan(clonedCol, 'span', maxColRange - minColRange)
        return clonedCol
      })
      var fakeColgroup = SugarElement.fromTag('colgroup')
  ...
  // jlx 获取行数据
  var getOriRows = function (house, minColRange, maxColRange) {
    return map(house.all, function (row) {
      var cellsToCopy = filter(row.cells, function (cell) {
        return cell.column >= minColRange && cell.column < maxColRange
      })
      var copiedCells = map(cellsToCopy, function (cell) {
        var clonedCell = cell.element
        //constrainSpan(clonedCell, 'colspan', maxColRange - minColRange)
        return clonedCell
      })
      //var fakeTR = SugarElement.fromTag('tr')
        ...
        each$1(items, function (itemValue, itemKey) {
          ...
          if (comparisonValue !== '' && key === itemKey) {
            if (comparisonValue !== itemValue) {
              if (baseData[key] instanceof String) {
                //字符串类型 改变为
                baseData[key] = ''
              }
            }
    ...
    return __assign(
      {
        ...
        padding: dom.getStyle(elm, 'padding') || dom.getAttrib(elm, 'padding'),
  ...
  var extractDataFromCellElement = function (editor, cell, hasAdvancedCellTab, column) {
    ...
    const breakLineDiv = cell.querySelector('[data-break-line]')
    const breakLine = getStyle(cell, 'white-space')
    const isBreakLine = breakLine == 'normal' || !!breakLineDiv
    console.log('isBreakLine', isBreakLine)
    const writingMode = getStyle(cell, 'writing-mode') //纵向排列
    return __assign(
      {
        width: getStyle(colElm, 'width'),
        padding: getStyle(colElm, 'padding'), //增加内边距
        height: getStyle(cell, 'height'),
        scope: dom.getAttrib(cell, 'scope'),
        breakLine: isBreakLine, // normal 是自动换行
        writingMode: writingMode != 'vertical-rl' ? false : true,
        tdContent: cell.innerText,
        // tdType: dom.getAttrib(cell, 'data-col-type') || '',
        celltype: getNodeName(cell),
        class: dom.getAttrib(cell, 'class', ''),
        halign: getHAlignment(editor, cell),
        valign: getVAlignment(editor, cell),
        overflow: getStyle(cell, 'overflow'),
        position: getStyle(cell, 'position')
      },
  ...
  var children$3 = [
    ...
    },
    {
      ...
      // "table": {},
      // "panel": {}
      name: 'tdContent', //单元格内容
      type: 'textarea', //改为textarea 为了保留span标签
      label: 'tdContent'
    },
    {
      name: 'scope',
  ...
  var updateSimpleProps = function (modifier, colModifier, data, elem, isSingleCell) {
    modifier.setAttrib('scope', data.scope)
    modifier.setAttrib('class', data.class)
    // jlx 设置单元格 斜线类型
    // modifier.setAttrib('cell-line-type', data.cellLineType)
    modifier.setStyle('height', addPxSuffix(data.height))
    colModifier.setStyle('width', addPxSuffix(data.width))
    if (data.padding) {
      modifier.setStyle('padding', addPxSuffix(data.padding))
    }

    colModifier.setStyle('overflow', addPxSuffix(data.overflow))
    colModifier.setStyle('position', addPxSuffix(data.position))
    const breakLineDiv = elem.querySelector('[data-break-line]')
    if (data.breakLine) {
      //自动换行
      if (!breakLineDiv) {
        const innerHTML = elem.innerHTML
        elem.innerHTML = '<div style="width:100%;height:inherit;overflow:hidden;line-height: 1;" data-break-line=1>' + innerHTML + '</div>'
      }
      modifier.setStyle('white-space', 'normal')
      modifier.setStyle('word-break', 'break-all') //兼容数字英文
    } else {
      if (breakLineDiv) {
        const innerHTML = breakLineDiv.innerHTML
        elem.innerHTML = innerHTML
      }
      modifier.setStyle('white-space', 'nowrap')
      modifier.setStyle('word-break', 'unset') //兼容数字英文
    }
    if (data.bordercolor) {
      elem.style.setProperty('--mce-border-color', data.bordercolor)
      // const lines = Array.from(elem.getElementsByTagName('line'))
      // lines.forEach(line=>{
      //   line.style.stroke = data.bordercolor
      // })
    }
    if (data.tdContent != '') {
      //将覆盖原来的td内部的数据
      const oriTdContent = elem.innerText
      if (data.breakLine) {
        elem.innerHTML = '<div style="width:100%;height:inherit;overflow:hidden;line-height: 1;" data-break-line=1>' + data.tdContent + '</div>'
      } else {
        elem.innerText = data.tdContent
      }
      //设置表格内容
    } else {
      if (isSingleCell) {
        //单独的单元格
        let pList = Array.from(elem.getElementsByTagName('p'))
        pList.forEach((pNode) => {
          elem.removeChild(pNode)
        })
      }
    }
    if (data.writingMode) {
      modifier.setStyle('writing-mode', 'vertical-rl')
      //佛山重症客户端 td内部不支持writing-mode
      const innerHTML = elem.innerHTML
      // data-vertical-rl="1" 必须有值   且data开头 否则保存后 再次进入缺少属性 影响值的回显
      elem.innerHTML = '<p style="writing-mode:vertical-rl; line-height:inherit; " data-vertical-rl="1">' + innerHTML + '</p>'
    } else {
      modifier.setStyle('writing-mode', 'unset')
      const verticalRlDom = elem.querySelectorAll('[data-vertical-rl]')
      if (verticalRlDom.length > 0) {
        elem.innerHTML = verticalRlDom[0].innerHTML || verticalRlDom[0].innerText
      }
    }
  }
  ...
  var applyCellData = function (editor, cells, oldData, data) {
    ...
    if (size(modifiedData) > 0 && cells.length >= 1) {
      ...
      getSelectedCells(cells).each(function (selectedCells) {
        each(selectedCells, function (item) {
          ...
          var colModifier = item.column
            ...
            .getOr(modifier)
          updateSimpleProps(modifier, colModifier, data, item.element, isSingleCell)
      ...
      tableOpt.each(function (table) {
        var colList = table.dom.querySelectorAll('col')
        var tableWidth = 0
        Array.from(colList).forEach((col) => {
          tableWidth += Number(col.style.width.split('px')[0])
        })
        //jlx 通过单元格设置宽度  表格的宽度跟随变化
        table.dom.style.width = tableWidth + 'px'
  ...
  var applyRowData = function (editor, rows, oldData, data) {
    ...
    if (size(modifiedData) > 0) {
      each(rows, function (rowElm) {
        ...
        var rowHeightChange = false // 行高发生变化
        //设置行高
        if(data.height){
          const tdList  = rowElm.querySelectorAll('td')
          const heightChange = Number(data.height.split('px')[0])-Number(oldData.height.split('px')[0]) //高度变化
          tdList.forEach(td => {
            td.style.height = data.height
          })
          table(SugarElement.fromDom(rows[0])).each(function (table) {
       
              console.log('table 00 ' ,table ,table.dom.style.height ,data.height ,oldData.height,heightChange)
              table.dom.style.height = (Number(table.dom.style.height.split('px')[0]) + heightChange) +'px'
              console.log('table 11' ,table ,table.dom.style.height ,data.height ,oldData.height)
          })
        }
  ...
  var applyDataToElement = function (editor, tableElm, data) {
    ...
    var startW = Number(tableElm.style.width.split('px')[0]) //获取调整之前的数据
    var startH = Number(tableElm.style.height.split('px')[0]) //获取调整之前的数据
    ...
    /** 表格设置宽高时 等比例设置对应每行每列数据  start */
    var table = SugarElement.fromDom(tableElm)
    var resizeTableWidth = function (table, width) {
      //表格属性设置后 col table的宽度等比例设置col的宽度
      var resizing = resizeHandler.lazyResizingBehaviour()
      var tableSize = get$9(editor, table)
      var col = isPreserveTableColumnResizing(editor) || isRightEdgeResize ? getGridSize(table).columns - 1 : 0
      adjustWidth(table, width - startW, col, resizing, tableSize)
    }
    var resizeTableHeight = function (table, newHeight) {
      //原有的表格属性设置只是单纯的调整了表格的style 未对表格的行高进行同步 导致在单独调整 行高是 表格又缩小或者放大
      ...
      const delta = rowList.map((row) => {
        const trH = Number(row.style.height.split('px')[0])
        return parseInt( newHeight * trH / startH - trH) //这里返回每行的步长
      })
      rowList.forEach((row, rIndex) => {
        const trH = parseInt(row.style.height.split('px')[0], 10)
        adjustHeight(table, delta[rIndex], rIndex, hdirection)
      })
    }
    resizeTableWidth(table, Number(data.width.split('px')[0]))
    resizeTableHeight(table, Number(data.height.split('px')[0]))
    /** 表格设置宽高时 等比例设置对应每行每列数据  end */
  ...
  var registerCommands = function (editor, actions, cellSelection, selections, clipboard) {
    ...
    var getTableFromCell = function (cell) {
      var tableInfo = table(cell, isRoot)
      return tableInfo
    }
    ...
    var pasteOnSelection = function (execute, getRows) {
      return getRows().each(function (rows) {
        ...
        var cellList = getSelectionStartCell$1(editor)
    ...
    each$1(
      {
        ...
        mceTablePasteColBefore: function (_grid) {
          // 将 TinyMCE 剪贴板行粘贴到当前行之前
          return pasteOnSelection(actions.pasteColsBefore, clipboard.getColumns)
        },
        mceTablePasteColAfter: function (_grid) {
          // 将 TinyMCE 剪贴板行粘贴到当前行之后
          return pasteOnSelection(actions.pasteColsAfter, clipboard.getColumns)
        },
        mceTablePasteRowBefore: function (_grid) {
          // 在当前行之前粘贴 TinyMCE 剪贴板列
          return pasteOnSelection(actions.pasteRowsBefore, clipboard.getRows)
        },
        mceTablePasteRowAfter: function (_grid) {
          // 在当前行之后粘贴 TinyMCE 剪贴板列
          return pasteOnSelection(actions.pasteRowsAfter, clipboard.getRows)
        },
        mceTableDelete: eraseTable, // 删除当前表
        mceTableSizingMode: function (ui, sizing) {
          // 当table_sizing_mode设置为 时'auto'，此命令设置当前选定表的大小调整模式
          return setSizingMode(sizing)
        },
        //jlx 添加插入元数据方法
        insertMetaDataFun: function (event) {
          var cells = getCellsFromSelection(getSelectionStart(editor), selections)
          editor.execCallback('insertMetaData_callback', cells)
        },
        //jlx 添加插入占位符方法
        insertPlaceholderFun: function (event) {
          var cells = getCellsFromSelection(getSelectionStart(editor), selections)
          editor.execCallback('insertPlacehoder_callback', cells)
        },
        // jlx 设置列头 放在列菜单中的 功能
        mceSetColumnHead: function (_grid) {
          getSelectionStartCell$1(editor).map(function (cell) {
            return getTableFromCell(cell).bind(function (table) {
              var target = forMenu(selections, table, cell)
              ...
              return details.map(function (selectedCells) {
                ...
                var minColRange = selectedCells[0].column
                var maxColRange = lastSelectedCell.column + lastSelectedCell.colspan
                //generateColGroup 生成colgroup数据 这里需要获取原始的数据
                var fakeColGroups = getOriColGroup(house, minColRange, maxColRange)
                fakeColGroups.map((col) => {
                  col.dom.setAttribute('data-col-type', 'colHeader')
                })
                var fakeRows = getOriRows(house, minColRange, maxColRange) //生成数据
                fakeRows.map((row) => {
                  var tdList = row
                  tdList.map((td) => {
                    td.dom.setAttribute('data-col-type', 'colHeader')
                  })
                })
              })
            })
        ...
        // jlx 设置列content 放在列菜单中的 功能
        mceSetColumnContent: function (_grid) {
          getSelectionStartCell$1(editor).map(function (cell) {
            return getTableFromCell(cell).bind(function (table) {
              var target = forMenu(selections, table, cell)
              ...
              return details.map(function (selectedCells) {
                ...
                var fakeColGroups = getOriColGroup(house, minColRange, maxColRange)
                fakeColGroups.map((col) => {
                  col.dom.setAttribute('data-col-type', 'colContent')
                })
                var fakeRows = getOriRows(house, minColRange, maxColRange) //生成数据
                fakeRows.map((row) => {
                  var tdList = row
                  tdList.map((td) => {
                    td.dom.setAttribute('data-col-type', 'colContent')
                  })
                })
              })
            })
         ...
         * @param {*} _grid 
         */
        mceSetRowspan: function (_grid) { 
          getSelectionStartCell$1(editor).map(function (cell) {
            const cellList = []
            let str = ''
            let list = ''
            getTableFromCell(cell).bind(function (table) {
              var target = forMenu(selections, table, cell)
              var house = Warehouse.fromTable(table)
              var details = onUnlockedCells(house, target)
              details.map(function (selectedCells) {
                var lastSelectedCell = selectedCells[selectedCells.length - 1]
                var minColRange = selectedCells[0].column
                var maxColRange = lastSelectedCell.column + lastSelectedCell.colspan
                //generateColGroup 生成colgroup数据 这里需要获取原始的数据
                var fakeColGroups = getOriColGroup(house, minColRange, maxColRange)
                fakeColGroups.map((col) => {
                  cellList.push(col.dom)
                  str = col.dom.getAttribute('data-row-span')
                  list = col.dom.getAttribute('data-rowspan-list')
                })
                var fakeRows = getOriRows(house, minColRange, maxColRange) //生成数据
                fakeRows.map((row) => {
                  var tdList = row
                  tdList.map((td) => {
                    cellList.push(td.dom)
                    str = td.dom.getAttribute('data-row-span')
                    list = td.dom.getAttribute('data-rowspan-list')
                  })
                })
              })
            })
            editor.execCallback('setRowSpan_callback', {dom: cellList, rule: str ,list :list})
          })
        },
        mceSetRowsplit: function (_grid) { 
          getSelectionStartCell$1(editor).map(function (cell) {
            return getTableFromCell(cell).bind(function (table) {
              var target = forMenu(selections, table, cell)
              ...
              return details.map(function (selectedCells) {
                ...
                var fakeColGroups = getOriColGroup(house, minColRange, maxColRange)
                fakeColGroups.map((col) => {
                  col.dom.removeAttribute('data-row-span')
                  col.dom.removeAttribute('data-rowspan-expression')
                  col.dom.removeAttribute('data-rowspan-list')
                })
                var fakeRows = getOriRows(house, minColRange, maxColRange) //生成数据
                fakeRows.map((row) => {
                  var tdList = row
                  tdList.map((td) => {
                    td.dom.removeAttribute('data-row-span')
                    td.dom.removeAttribute('data-rowspan-expression')
                    td.dom.removeAttribute('data-rowspan-list')
                  })
                })
              })
            })
    ...
    each$1(
      {
        mceTableRowType: function () {
          // 将当前行或行更改为指定的类型：'header'、'body'或'footer'
          return actions.getTableRowType(editor)
        },
        mceTableCellType: function () {
          // 将当前一列或多列中的所有单元格更改为指定类型，或者：'td'或'th'
  ...
  var adt$6 = Adt.generate([{ none: ['message'] }, { success: [] }, { failedUp: ['cell'] }, { failedDown: ['cell'] }])
  ...
  var getSelectionTargets = function (editor, selections) {
    ...
    var onSetupInsertTable = function (api) {
      // jlx 插入表格的条件 设计页面只允许增加一个表格组件
      const bodyDOM = editor.getBody()
      const tableList = bodyDOM.getElementsByTagName('table')
      const isHaveTable = tableList.length > 0
      return api.setDisabled(isHaveTable)
    }
     ...
     * 拆分列表的条件
     */
    var onSetupSplitRow = function (api) {
      return onSetup(api, function (_targets) {
        let flag = true //有data-row-span 的情况可以拆分
        let noHaveRowspan = []
        let haveRowspan = []
        let count = 0
        getSelectionStartCell$1(editor).map(function (cell) {
          ...
          tableInfo(cell).bind(function (table) { 
            var target = forMenu(selections, table, cell)
            var house = Warehouse.fromTable(table)
            var details = onUnlockedCells(house, target)
            return details.map(function (selectedCells) { 
              ...
              var fakeColGroups = getOriColGroup(house, minColRange, maxColRange)
              fakeColGroups.map((col) => {
                if (col.dom.getAttribute('data-row-span') == null){  //没有该属性就没有拆分权限 不显示拆分
                  noHaveRowspan.push(col.dom)
                }
        ...
        return noHaveRowspan.length>0
      })
    }
    /**
     * 合并列表的条件
     */
    var onSetupMergeRow = function (api) {
      return onSetup(api, function (_targets) {
        let flag = true
        getSelectionStartCell$1(editor).map(function (cell) {
          var isRoot = getIsRoot(editor)
          ...
          tableInfo(cell).bind(function (table) { 
            ...
            return details.map(function (selectedCells) { 
              ...
              var fakeColGroups = getOriColGroup(house, minColRange, maxColRange)
              fakeColGroups.map((col) => {
                if (col.dom.getAttribute('data-row-span') != null) { 
                  flag = false
                }
              })
            })
    ...
    return {
      ...
      onSetupInsertTable: onSetupInsertTable,
      onSetupSplitRow: onSetupSplitRow,
      onSetupMergeRow: onSetupMergeRow,
      targets: function () {
        return targets.get()
      }
    }
    ...
    editor.ui.registry.addMenuButton('table', {
      ...
      fetch: function (callback) {
        if (hasTableGrid(editor) === false) {
          ...
          return callback('inserttable customtable | cell row column | advtablesort | tableprops deletetable')
        }
      }
    })
  ...
  var addMenuItems = function (editor, selectionTargets, clipboard) {
    ...
    var insertMetaData = {
      ...
      text: 'insertMetaData',
      onSetup: selectionTargets.onSetupTable,
      onAction: cmd('insertMetaDataFun')
    }
    ...
    var insertPlaceHolder = {
      ...
      text: '插入占位符',
      onSetup: selectionTargets.onSetupTable,
      onAction: cmd('insertPlaceholderFun')
    }
    ...
    //设置列头
    editor.ui.registry.addMenuItem('setcolumnhead', {
      text: '设置列头',
      icon: 'column-header',
      onAction: cmd('mceSetColumnHead')
    })
    //设置横向表格列内容
    editor.ui.registry.addMenuItem('setcolumncontent', {
      text: '设置列内容',
      icon: 'column-content',
      onAction: cmd('mceSetColumnContent')
    })
    //设置表格列纵向合并
    editor.ui.registry.addMenuItem('setrowspan', {
      text: '设置纵向合并规则',
      // icon: 'column-span',
      onAction: cmd('mceSetRowspan'),
      // onSetup:selectionTargets.onSetupMergeRow  为了回显 合并规则
    })
    // 拆分列的合并
    editor.ui.registry.addMenuItem('setrowsplit', {
      text: '拆分纵向合并',
      // icon: 'column-span',
      onAction: cmd('mceSetRowsplit'),
      onSetup: selectionTargets.onSetupSplitRow
    })
    var column = {
      type: 'nestedmenuitem',
      text: 'Column',
      getSubmenuItems: function () {
        return ' setcolumnhead setcolumncontent | setrowspan setrowsplit | tableinsertcolumnbefore tableinsertcolumnafter tabledeletecolumn |  tablecutcolumn tablecopycolumn tablepastecolumnbefore tablepastecolumnafter'
      }
    }
    editor.ui.registry.addMenuItem('createrowdata', {
      text: '智能填充',
      onAction: function () {
        ...
        var data = getData(editor, cells)
        editor.undoManager.transact(function () {
          createFormatData(editor, cells) //格式化对应的单元格数据
          editor.focus()
        })
      },
      onSetup: selectionTargets.onSetupCellOrRow
    ...
    editor.addCommand('addcellslash',function(){
      var selections = Selections(
        function () {
          return getBody$1(editor)
        },
      ...
      editor.undoManager.transact(function () {
        addCellSlash(editor, cells, data, false, false, true, false)
        editor.focus()
      })
    })
    // jlx 添加斜线在添加菜单
    editor.ui.registry.addMenuItem('addcellslash', {
      text: 'Add cell Slash',
      icon: 'cell-slash',
      onAction: cmd('addcellslash'),
      onSetup: selectionTargets.onSetupCellOrRow
    })
    // wyw 添加反斜线在添加菜单
    editor.ui.registry.addMenuItem('addcellbackslash', {
      text: 'Add cell Back Slash',
      icon: 'cell-slash-back',
      onAction: function () {
        var selections = Selections(
          function () {
            return getBody$1(editor)
        ...
        editor.undoManager.transact(function () {
          addCellSlash(editor, cells, data, true, false, true, false)
          editor.focus()
        })
      },
      onSetup: selectionTargets.onSetupCellOrRow
    })
    editor.addCommand('diagonalslash',function(){
      var selections = Selections(
        function () {
          return getBody$1(editor)
        },
      ...
      editor.undoManager.transact(function () {
        addCellSlash(editor, cells, data)
        editor.focus()
      })
    })
    // wyw 添加斜线(对角线)在添加菜单
    editor.ui.registry.addMenuItem('addcelldiagonalslash', {
      text: 'Add cell Diagonal Slash',
      icon: 'cell-diagonal',
      onAction: cmd('diagonalslash'),
      onSetup: selectionTargets.onSetupCellOrRow
    })
    // wyw 添加反斜线(对角线)在添加菜单
    editor.ui.registry.addMenuItem('addcelldiagonalbackslash', {
      text: 'Add cell Diagonal Back Slash',
      icon: 'cell-diagonal-back',
      onAction: function () {
        var selections = Selections(
          function () {
            return getBody$1(editor)
        ...
        editor.undoManager.transact(function () {
          addCellSlash(editor, cells, data, true)
          editor.focus()
        })
      },
      onSetup: selectionTargets.onSetupCellOrRow
    ...
    // wyw 添加双斜线在添加菜单
    editor.ui.registry.addMenuItem('addcelldoubleslash', {
      text: 'Add cell Double Slash',
      icon: 'cell-double-slash',
      onAction: function () {
        var selections = Selections(
          function () {
            return getBody$1(editor)
        ...
        editor.undoManager.transact(function () {
          addCellSlash(editor, cells, data, false, true)
          editor.focus()
        })
      },
      onSetup: selectionTargets.onSetupCellOrRow
    ...
    // jlx 添加双斜线在添加菜单
    editor.ui.registry.addMenuItem('addcelldoubleslash1', {
      text: '添加双斜线',
      icon: 'cell-double-specail',
      onAction: function () {
        var selections = Selections(
          function () {
            return getBody$1(editor)
        ...
        editor.undoManager.transact(function () {
          if (cells.length >= 1) {
            getSelectedCells(cells).each(function (selectedCells) {
              each(selectedCells, function (item) {
                var cellElm = switchCellType(editor, item.element, data.celltype)
                cellElm.setAttribute('class', 'double-slash slash-specail')

                cellElm.setAttribute('data-mce-style', `${cellElm.getAttribute('data-mce-style') || ''}`)
              })
            })
          }
          editor.focus()
    ...
    // wyw 添加双反斜线在添加菜单
    editor.ui.registry.addMenuItem('addcelldoublebackslash', {
      text: 'Add cell Double Back Slash',
      icon: 'cell-double-slash-back',
      onAction: function () {
        var selections = Selections(
          function () {
            return getBody$1(editor)
        ...
        editor.undoManager.transact(function () {
          addCellSlash(editor, cells, data, true, true)
          editor.focus()
        })
      },
      onSetup: selectionTargets.onSetupCellOrRow
    ...
    // wyw 添加双斜线(底部)在添加菜单
    editor.ui.registry.addMenuItem('addcellbottomdoubleslash', {
      text: 'Add cell Bottom Double Slash',
      icon: 'cell-bottom-double-slash',
      onAction: function () {
        var selections = Selections(
          function () {
            return getBody$1(editor)
        ...
        editor.undoManager.transact(function () {
          addCellSlash(editor, cells, data, false, true, false)
          editor.focus()
        })
      },
      onSetup: selectionTargets.onSetupCellOrRow
    ...
    // wyw 添加双反斜线(底部)在添加菜单
    editor.ui.registry.addMenuItem('addcellbottomdoublebackslash', {
      text: 'Add cell Bottom Double Back Slash',
      icon: 'cell-bottom-double-slash-back',
      onAction: function () {
        var selections = Selections(
          function () {
            return getBody$1(editor)
        ...
        editor.undoManager.transact(function () {
          addCellSlash(editor, cells, data, true, true, false)
          editor.focus()
        })
      },
      onSetup: selectionTargets.onSetupCellOrRow
    ...
    editor.addCommand('deletecellslash',function(){
      var selections = Selections(
        function () {
          return getBody$1(editor)
        },
      ...
      editor.undoManager.transact(function () {
        deleteCellSlash(editor, cells, data)
        editor.focus()
      })
    })
    // jlx 删除斜线
    editor.ui.registry.addMenuItem('deletecellslash', {
      text: 'Delete cell Slash',
      icon: 'delete-cell-slash',
      onAction: cmd('deletecellslash'),
      onSetup: selectionTargets.onSetupCellOrRow
    })
    
    
    // 交叉表中对选中的单元格数据进行格式化填充 智能填充
    var createFormatData = function (editor, cells) {
      var oriTdContent = cells[0].dom.innerText
      var tdContent = cells[0].dom.innerText
      var tdInnerHTML = cells[0].dom.innerHTML
      var dayjs = editor.getParam('dayjs')
      var step = 0
      var today = dayjs().format('YYYY-MM-DD ')
      if (!tdContent) return
      const timeRegex = /([0-1][0-9]|[2][0-3]):([0-5][0-9])(:([0-5][0-9]))?/ //正则表达式  hh:mm:ss 或者  hh:mm
      if (timeRegex.test(tdContent)) {
        //时间格式
        const timeStr = tdContent.match(timeRegex)[0]
        let timeFormatStr = 'HH:mm:ss'
        cells = cells.map((td, index) => {
          let currTdContent = td.dom.innerText //当前
          currTdContent = currTdContent.replaceAll('\n', '')
          if (index == 1 && currTdContent) {
            step = dayjs(today + currTdContent.match(timeRegex)[0]).valueOf() - dayjs(today + timeStr).valueOf()
            timeFormatStr = timeFormatStr.split(':').splice(0, timeStr.split(':').length).join(':')
          }
          if (index > 0 && currTdContent == '') {
            td.dom.innerHTML = tdInnerHTML.replaceAll(timeStr, dayjs(dayjs(today + timeStr).valueOf() + index * step).format(timeFormatStr))
          }
          return td
        })
      }

      if (/[1-9][0-9]*/.test(tdContent) && step == 0) {
        tdContent = cells[0].dom.innerText
        tdInnerHTML = cells[0].dom.innerHTML
        var numArr = tdContent.match(/[1-9][0-9]*/g) //数字分组
        cells.forEach((td, index) => {
          let currTdContent
          currTdContent = td.dom.innerText //当前
          currTdContent = currTdContent.replaceAll('\n', '')
          if (index == 1 && currTdContent) {
            var tempNumArr = currTdContent.match(/[1-9][0-9]*/g)
            if (tempNumArr.length > 1) {
              step = []
              tempNumArr.forEach((n, i) => {
                if (parseInt(tempNumArr[i] || '0') - parseInt(numArr[i] || '0')) {
                  step.push(parseInt(tempNumArr[i] || '0') - parseInt(numArr[i] || '0'))
                } else {
                  step.push(0)
                }
              })
            } else step = parseInt(tempNumArr[0]) - parseInt(numArr[0], 10)
          }
          if (Array.isArray(step)) {
            //多个数字 多个步长
            numArr.forEach((numberStr, n) => {
              const strArr = tdContent.split(numberStr)
              const s = step[n]
              if (timeRegex.test(tdContent)) {
                let content = td.dom.innerText
                td.dom.innerHTML = content.replaceAll(strArr[n] + numArr[n], isNaN(s) ? strArr[n] + parseInt(numArr[sIndex]) : strArr[n] + (parseInt(numArr[n]) + parseInt(index * s)))
              } else {
                td.dom.innerHTML = tdInnerHTML.replaceAll(strArr[n] + numArr[n], isNaN(s) ? strArr[n] + parseInt(numArr[sIndex]) : strArr[n] + (parseInt(numArr[n]) + parseInt(index * s)))
              }
            })
          } else {
            td.dom.innerHTML = tdInnerHTML.replaceAll(numArr[0], isNaN(step) ? parseInt(numArr[0]) : parseInt(numArr[0]) + parseInt(index * step))
          }
        })
      }
    }
    ...
    // jlx 添加斜线
    var addCellSlash = function (editor, cells, data, isBack = false, isDouble = false, isTop = true, isDiagonal = true) {
      // 获取单元格属性
      if (cells.length >= 1) {
        getSelectedCells(cells).each(function (selectedCells) {
          each(selectedCells, function (item) {
            var cellElm = switchCellType(editor, item.element, data.celltype)
            let classStr = cellElm.getAttribute('class')||''
            if (isDouble) {
              // 是否双斜线
              if (isTop) {
                // 是否从顶部发出斜线
                if (isBack) {
                  if(classStr.indexOf('double-slash slash-top-left')==-1) classStr += ' double-slash slash-top-left'
                  cellElm.setAttribute('class', classStr)
                } else {
                  if(classStr.indexOf('double-slash slash-top-right')==-1) classStr += ' double-slash slash-top-right'
                  cellElm.setAttribute('class', classStr)
                }
              } else {
                if (isBack) {
                  if(classStr.indexOf('double-slash slash-bottom-right')==-1) classStr += ' double-slash slash-bottom-right'
                  cellElm.setAttribute('class', classStr)
                } else {
                  if(classStr.indexOf('double-slash slash-bottom-left')==-1) classStr += ' double-slash slash-bottom-left'
                  cellElm.setAttribute('class', classStr)
                }
              }
            } else {
              if (isDiagonal) {
                // 是否是对角线
                if (isBack) {
                  if(classStr.indexOf('slash reverse-diagonal-slash')==-1) classStr += ' slash reverse-diagonal-slash'
                  cellElm.setAttribute('class', classStr)
                } else {
                  if(classStr.indexOf('slash diagonal-slash')==-1) classStr += ' slash diagonal-slash'
                  cellElm.setAttribute('class', classStr)
                }
              } else {
                if (isBack) {
                  if(classStr.indexOf('slash reverse-single-slash')==-1) classStr += ' slash reverse-single-slash'
                  cellElm.setAttribute('class', classStr)
                } else {
                  if(classStr.indexOf('slash single-slash')==-1) classStr += ' slash single-slash'
                  cellElm.setAttribute('class', classStr)
                }
              }
            }
    ...
    // jlx 删除斜线
    var deleteCellSlash = function (editor, cells, data) {
      // 获取单元格属性
      if (cells.length >= 1) {
        getSelectedCells(cells).each(function (selectedCells) {
          each(selectedCells, function (item) {
            var cellElm = switchCellType(editor, item.element, data.celltype)
            const childDomList = cellElm.childNodes
            for (let i = childDomList.length - 1; i >= 0; i--) {
              const dom = childDomList[i]
              if (dom.getAttribute) {
                // 兼容老版本斜线删除
                const newCellSlashArr = dom.getAttribute('data-mce-cellslash-new')
                // 兼容老版本斜线删除
                const cellSlashArr = dom.getAttribute('data-mce-cellslash-parent')
                // 兼容老版本斜线删除
                const oldCellSlashArr = dom.getAttribute('data-mce-cellslash')
                if ((dom.nodeName == 'DIV' || dom.nodeName == 'svg') && (cellSlashArr != null || oldCellSlashArr != null || newCellSlashArr != null)) {
                  dom.remove(dom)
                }
              }
            }
            ...
            //需要删除斜线相关的 css 
            let classStr = cellElm.getAttribute('class')
            classStr = classStr.replace('double-slash slash-top-left',' ')
            classStr = classStr.replace('double-slash slash-top-right',' ')
            classStr = classStr.replace('double-slash slash-bottom-right',' ')
            classStr = classStr.replace('double-slash slash-bottom-left',' ')

            classStr = classStr.replace('slash reverse-diagonal-slash',' ')
            classStr = classStr.replace('slash diagonal-slash',' ')
            classStr = classStr.replace('slash reverse-single-slash',' ')
            classStr = classStr.replace('slash single-slash',' ')
            cellElm.setAttribute('class', classStr)
          })
        })
      }
     ...
     * 添加清除单元格方法
     */
    editor.addCommand('deletecellborder',function(){
      var selections = Selections(
        function () {
          return getBody$1(editor)
        },
      ...
      editor.undoManager.transact(function () {
        addCellBorder(editor, cells, data, 'none')
        editor.focus()
      })
    })
    /**
     * 添加单元格边线
     */
    editor.addCommand('addcellborder',function(){
      var selections = Selections(
        function () {
          return getBody$1(editor)
        },
      ...
      editor.undoManager.transact(function () {
        addCellBorder(editor, cells, data)
        editor.focus()
      })
    })
    /**
     * 添加单元格边top线
     */
    editor.addCommand('addcellbordertop',function(){
      var selections = Selections(
        function () {
          return getBody$1(editor)
        },
      ...
      editor.undoManager.transact(function () {
        addCellBorder(editor, cells, data, 'top')
        editor.focus()
      })
    })
    /**
     * 添加单元格边left线
     */
    editor.addCommand('addcellborderleft',function(){
      var selections = Selections(
        function () {
          return getBody$1(editor)
        },
      ...
      editor.undoManager.transact(function () {
        addCellBorder(editor, cells, data, 'left')
        editor.focus()
      })
    })
    /**
     * 添加单元格边bottom线
     */
    editor.addCommand('addcellborderbottom',function(){
      var selections = Selections(
        function () {
          return getBody$1(editor)
        },
      ...
      editor.undoManager.transact(function () {
        addCellBorder(editor, cells, data, 'bottom')
        editor.focus()
      })
    })
    /**
     * 添加单元格边around线
     */
    editor.addCommand('addcellborderaround',function(){
      var selections = Selections(
        function () {
          return getBody$1(editor)
        },
      ...
      editor.undoManager.transact(function () {
        addCellBorder(editor, cells, data, 'around')
        editor.focus()
      })
    })
    /**
     * 添加单元格边right线
     */
    editor.addCommand('addcellborderright',function(){
      var selections = Selections(
        function () {
          return getBody$1(editor)
        },
      ...
      editor.undoManager.transact(function () {
        addCellBorder(editor, cells, data, 'right')
        editor.focus()
      })
    })
    // wyw 添加单元格边框在添加菜单
    editor.ui.registry.addMenuItem('addcellborder', {
      text: 'Add Cell Border',
      icon: 'cell-border-all',
      onAction: cmd('addcellborder'),
      onSetup: selectionTargets.onSetupCellOrRow
    })
    // wyw 添加单元格上边框在添加菜单
    editor.ui.registry.addMenuItem('addcellbordertop', {
      text: 'Add Cell Border Top',
      icon: 'cell-border-top',
      onAction: cmd('addcellbordertop'),
      onSetup: selectionTargets.onSetupCellOrRow
    })
    // wyw 添加单元格左边框在添加菜单
    editor.ui.registry.addMenuItem('addcellborderleft', {
      text: 'Add Cell Border Left',
      icon: 'cell-border-left',
      onAction: cmd('addcellborderleft'),
      onSetup: selectionTargets.onSetupCellOrRow
    })
    // wyw 添加单元格右边框在添加菜单
    editor.ui.registry.addMenuItem('addcellborderright', {
      text: 'Add Cell Border Right',
      icon: 'cell-border-right',
      onAction: cmd('addcellborderright'),
      onSetup: selectionTargets.onSetupCellOrRow
    })
    // wyw 添加单元格下边框在添加菜单
    editor.ui.registry.addMenuItem('addcellborderbottom', {
      text: 'Add Cell Border Bottom',
      icon: 'cell-border-bottom',
      onAction: cmd('addcellborderbottom'),
      onSetup: selectionTargets.onSetupCellOrRow
    })
    // wyw 添加清除单元格边框在添加菜单
    editor.ui.registry.addMenuItem('addcellborderclear', {
      text: 'Add Cell Border Clear',
      icon: 'cell-border-clear',
      onAction: cmd('deletecellborder'),
      onSetup: selectionTargets.onSetupCellOrRow
    })
    // wyw 添加单元格外侧边框在添加菜单
    editor.ui.registry.addMenuItem('addcellborderaround', {
      text: 'Add Cell Border Around',
      icon: 'cell-border-around',
      onAction: cmd('addcellborderaround'),
      onSetup: selectionTargets.onSetupCellOrRow
    })
    // wyw 添加边框
    var addCellBorder = function (editor, cells, data, position = 'all') {
      // 获取单元格属性
      if (cells.length >= 1) {
        var cellElmArr = [],
          lastRowIndex = null,
          lastCellIndex = null
        getSelectedCells(cells).each(function (selectedCells) {
          const rowMinIndex = selectedCells[0].element.parentElement.rowIndex,
            rowMaxIndex = selectedCells[selectedCells.length - 1].element.parentElement.rowIndex,
            colMinIndex = selectedCells[0].element.cellIndex,
            colMaxIndex = selectedCells[selectedCells.length - 1].element.cellIndex
          each(selectedCells, function (item) {
            var cellElm = switchCellType(editor, item.element, data.celltype)
            cellElmArr.push({ item: cellElm, rowIndex: item.element.parentElement.rowIndex, cellIndex: item.element.cellIndex })
            let varStyle = ''
            if (position == 'top') {
              // 上边框
              if (rowMinIndex === item.element.parentElement.rowIndex) {
                varStyle = addBorderStyle(cellElm, 'top')
              }
            } else if (position == 'left') {
              // 左边框
              if (colMinIndex === item.element.cellIndex) {
                varStyle = addBorderStyle(cellElm, 'left')
              }
            } else if (position == 'right') {
              // 右边框
              if (colMaxIndex === item.element.cellIndex) {
                varStyle = addBorderStyle(cellElm, 'right')
              }
            } else if (position == 'bottom') {
              // 下边框
              if (rowMaxIndex === item.element.parentElement.rowIndex) {
                varStyle = addBorderStyle(cellElm, 'bottom')
              }
            } else if (position == 'none') {
              // 无边框
              varStyle = addBorderStyle(cellElm, 'none')
              // cellElm.setAttribute('class', `${cellElm.getAttribute('class') || ''} mce-item-td`)
              ...
              const oldClass = cellElm.getAttribute('class') || ''
              cellElm.setAttribute('class', `${oldClass.replace(' mce-item-td', '')}`)
            } else {
              // 全边框
              cellElm.style.borderTopStyle = 'solid'
              cellElm.style.borderLeftStyle = 'solid'
              cellElm.style.borderRightStyle = 'solid'
              cellElm.style.borderBottomStyle = 'solid'
              cellElm.style.borderTopWidth = '1.1px'
              cellElm.style.borderLeftWidth = '1.1px'
              cellElm.style.borderRightWidth = '1px'
              cellElm.style.borderBottomWidth = '1px'
              cellElm.style.borderColor = 'inherit'
              varStyle = 'border-top-style: solid;border-left-style: solid;border-right-style: solid;border-bottom-style: solid;'
              const oldClass = cellElm.getAttribute('class') || ''
              cellElm.setAttribute('class', `${oldClass.replace(' mce-item-td', '')}`)
            }
            if (position == 'none') {
              ...
              const oldClass = cellElm.getAttribute('class') || ''
              if (oldClass != '') {
                //  手动设置全边框 去掉mce-item-td 样式
                const topBrStyle = cellElm.style.borderTopStyle
                const rightBrStyle = cellElm.style.borderRightStyle
                const bottomBrStyle = cellElm.style.borderBottomStyle
                const leftBrStyle = cellElm.style.borderLeftStyle
                if (topBrStyle == 'solid' && rightBrStyle == 'solid' && bottomBrStyle == 'solid' && leftBrStyle == 'solid') {
                  cellElm.setAttribute('class', `${oldClass.replace(' mce-item-td', '')}`)
                }
              }
            }
            cellElm.setAttribute('data-mce-style', `${varStyle}`)
          })
        })
      }
    }
    var addBorderStyle = function (ele, position) {
      if (['top', 'left', 'right', 'bottom'].includes(position)) {
        const capitalPosition = position.toLowerCase().replace(/( |^)[a-z]/g, (L) => L.toUpperCase())
        ele.style[`border${capitalPosition}Style`] = 'solid'
        /**
         * 表格单元格设置了'border-collapse: collapse;'，
         * 单元格周边的单元格清除边框，由于清除边框时设置了1px的透明边框，
         * collapse属性会以取右边框或下边框的样式为主合并边框，
         * 该单元格设置上边框或左边框时会出现没有效果的情况，
         * 边框宽度设置为1.1px时，会优先取边框宽度大一点的边框样式，可避免此情况的发生
         */
        if (['top', 'left'].includes(position)) {
          ele.style[`border${capitalPosition}Width`] = '1.1px'
        } else {
          ele.style[`border${capitalPosition}Width`] = '1px'
        }
        ele.style[`border${capitalPosition}Color`] = 'inherit'
        return `border-${position}-style: solid;border-${position}-width: 1px;border-${position}-color: inherit;`
      } else {
        ele.style.borderStyle = ''
        ele.style.borderWidth = ''
        ele.style.borderColor = ''
        ele.style.borderTopStyle = ''
        ele.style.borderTopWidth = ''
        ele.style.borderTopColor = ''
        ele.style.borderBottomStyle = ''
        ele.style.borderBottomWidth = ''
        ele.style.borderBottomColor = ''
        ele.style.borderLeftStyle = ''
        ele.style.borderLeftWidth = ''
        ele.style.borderLeftColor = ''
        ele.style.borderRightStyle = ''
        ele.style.borderRightWidth = ''
        ele.style.borderRightColor = ''
        return 'border-top: none;border-left: none;border-right: none;border-bottom: none;'
      }
    }
    // jlx 在对应的表格设置单元格
    ...
    var cell = {
      ...
      text: 'Cell',
      getSubmenuItems: function () {
        const type = getCriticlalRecordTypeDict(editor)
        if (type == '1') {
          return 'tablecellprops createrowdata tablemergecells tablesplitcells addcellslash addcellbackslash addcelldiagonalslash addcelldiagonalbackslash addcelldoubleslash addcelldoublebackslash addcellbottomdoubleslash addcellbottomdoublebackslash deletecellslash addcellborderclear addcellborder addcellborderaround addcellbordertop addcellborderleft addcellborderright addcellborderbottom'
        } else {
          return 'tablecellprops tablemergecells tablesplitcells addcellslash addcellbackslash addcelldiagonalslash addcelldiagonalbackslash addcelldoubleslash addcelldoublebackslash addcellbottomdoubleslash addcellbottomdoublebackslash deletecellslash addcellborderclear addcellborder addcellborderaround addcellbordertop addcellborderleft addcellborderright addcellborderbottom'
        }
        // return 'tablecellprops tablemergecells tablesplitcells addcellslash addcellbackslash addcelldiagonalslash addcelldiagonalbackslash deletecellslash addcellborderclear addcellborder addcellborderaround addcellbordertop addcellborderleft addcellborderright addcellborderbottom'
      }
    }
    //需要优化   如果grid中没有对应的行列 可以触发输入框 jlx
    if (hasTableGrid(editor) === false) {
      editor.ui.registry.addMenuItem('inserttable', {
        ...
        icon: 'table',
        onAction: cmd('mceInsertTable'),
        onSetup: selectionTargets.onSetupInsertTable
      })
    } else {
      ...
      //自定义表格
      editor.ui.registry.addMenuItem('customtable', {
        //菜单项 插入表格弹框菜单
        text: '插入表格',
        icon: 'table',
        onAction: cmd('mceInsertTable'),
    ...
    editor.ui.registry.addMenuItem('insertMetaData', insertMetaData)
    editor.ui.registry.addMenuItem('insertPlaceHolder', insertPlaceHolder)
        ...
        return selectionTargets.targets().fold(
          ...
          function (targets) {
            if (name(targets.element) === 'caption') {
              ...
              return 'cell row column | advtablesort | tableprops insertMetaData  deletetable insertPlaceHolder'
            }
          }
        )
      }
