...
(function () {
    ...
    var waitLoadImage = function (editor, data, imgElm) {
      var selectImage = function () {
        ...
        if (editor.selection) {
          ...
          imgElm.setAttribute('onerror', data.onerror)
    ...
    var getImageOnerror = function (image) {
      return getAttrib(image, 'onerror');
    };
    ...
    var read = function (normalizeCss, image) {
      return {
        src: getAttrib(image, 'data-mce-src'),
    ...
    var setAlt = function (image, alt, isDecorative) {
      if (isDecorative) {
        ...
        set(sugarImage, 'alt', 'image');
      } else {
        
        var sugarImage = SugarElement.fromDom(image);
        set(sugarImage, 'alt', alt||'image');
    ...
    var setOnerror = function (image, onerror) {
      var sugarImage = SugarElement.fromDom(image);
      set(sugarImage, 'onerror', onerror);
    };
    ...
    var write = function (normalizeCss, newData, image) {
      ...
      updateProp(image, oldData, newData, 'onerror', updateAttrib);
    ...
    var insertImageAtCaret = function (editor, data) {
      ...
      if (isFigure(insertedElm)) {
        ...
        editor.dom.setAttrib(insertedElm, 'onerror', data.onerror);
    ...
    var toImageData = function (data, removeEmptyAlt) {
      return {
        src: data.src.value,
        onerror: data.onerror,
        alt: data.alt.length === 0 && removeEmptyAlt ? 'image' : data.alt,
        title: data.title,
