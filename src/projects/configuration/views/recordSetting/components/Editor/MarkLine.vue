<template>
  <div class="mark-line" @click.stop>
    <div v-for="line in lines" :key="line" class="line" :class="line.includes('x') ? 'xline' : 'yline'" :ref="line"
  ...
  methods: {
    ...
    translateComponentStyle(style) {
      ...
      if (style.rotate != 0) {
        ...
        style.bottom = Number(style.top) + Number(style.height)
        style.right = Number(style.left) + Number(style.width)
      }
