...
import {
  wlStatistics_wardWorkLoadStatics
} from '@/api/qcReport'
  ...
  methods: {
    ...
    * */
    ...
    getData() {
      ...
      wlStatistics_wardWorkLoadStatics(query).then(pro => {
        pro.data = pro.data.data || {
          xaxis: [{ data: [] }], max: 18,
          min: 0, series: []
        }
        this.option.xAxis[0].data = pro.data.xaxis
        this.option.yAxis = [{ min: pro.data.min, max: pro.data.max }]
        this.option.series[0].data = pro.data.series[0]
