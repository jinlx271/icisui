  ...
  <div class="HCContiner">
    <div class="Header flex align-items-center">
      <div class="title">
        ...
        <span class="system">{{ globalConfig.title }}</span>
      </div>
      <div class="logout">
    ...
    <div class="LMContiner">
      <el-menu :default-active="activeIndex" @select="handleSelect" class="el-menu-vertical-demo leftMenu" ref="menu">
        <div v-for="(item, index) in mainMenu" :key="item.path">
          <router-link :ref="`menuPathList${item.path}`"
            :to="{ path: item.path, query: { menuId: item.id, width: width, height: height } }" v-if="showItem(item)">
      ...
      <div class="Main flex-1">
        <van-pull-refresh v-model="vanPull" @refresh="onRefresh">
...
export default {
  ...
  computed: {
    ...mapGetters(['currentUserWard', 'userWardList', 'mainMenu'])
  },
  mounted() {
    console.log('this.activeIndex', this.mainMenu)
